...
function ParseDate(dateStr: string): Date | null {
  ...
  if (shortParts) {
    const tempDate = new Date(
      `${shortParts[2]} ${shortParts[1]} ${shortParts[3]}`
    );
    // Use UTC for this format as well
    return new Date(
      Date.UTC(
...
async function NotifyOnEndOfSupport(
  ...
  logger.info(
    `EOL notification for ${versionData.ProductName} ${versionData.VersionName} will be handled in batch processing`
  );
  return;
}
...
async function ExtractFortraVersions(
  productname: string,
  listoffortraversions: any
): Promise<VersionExtracted[]> {
  ...
  try {
    ...
    for (let i = 0; i < listoftd.length; i += 7) {
      ...
      switch (cheerio.load(listofVersions)(listoftd[i]).text().toLowerCase()) {
        case "mft":
          VersionObject = FortraVersionObjectCreator(
            cheerio
              .load(listofVersions)(listoftd[i + 1])
              .text(),
            cheerio
              .load(listofVersions)(listoftd[i + 3])
              .text(),
            cheerio
              .load(listofVersions)(listoftd[i + 6])
              .text(),
            cheerio
              .load(listofVersions)(listoftd[i + 2])
              .text(),
            cheerio
              .load(listofVersions)(listoftd[i + 5])
              .text(),
            cheerio
              .load(listofVersions)(listoftd[i + 4])
              .text()
          );
...
async function NotifyOnEndOfSupportChanges(
  product: string,
  vendor: string,
  version: string,
  oldDate?: Date,
  ...
  const emailBody = EmailBodyCreator(
    "Team",
    `End of Support Date Change: ${product.replace(/_/g, " ")} ${version}`,
    `Hey Team`,
    `The end of support date for ${product.replace(
      /_/g,
      " "
    )} ${version} has been changed.`,
    `Changes Detected:`,
    `End of Support Date changed from `,
    `${oldDate ? oldDate.toDateString() : "No old date"}`,
    `to`,
    `${newDate ? newDate.toDateString() : "No new date"}`
  );

  try {
    await SendEmail({
      subject: `End of Support Date Change: ${product.replace(
        /_/g,
        " "
      )} ${version}`,
      content: emailBody,
...
async function ExtractOpswatKeyIndexes(url: string) {
  try {
    ...
    while (!elementfound) {
      ...
      relevant_obj = relevant_obj.publicDocumentations.find(
        (element: any) =>
          element.slug.includes("knowledgebase") ||
          element.slug.includes("knowledge-base")
      );
      ...
      if (relevant_obj?.publicIndxes) {
        ...
        for (let i = 0; i < objwithpageonly.length; i++) {
          if (objwithpageonly[i].page !== undefined) {
            const regex =
              /how-long-is-the-support-lifecycle-for-a-specific-version-of-|how-long-is-the-support-life-cycle-for-a-/;
    ...
    const concated_indexes: string[] = arrayofpages.map(
...
function ExtractVersionsFromJson(
  response_json: any,
  manufacturer: string,
  productName: string
): VersionExtracted[] {
  ...
  if (manufacturer === "OPSWAT") {
    if (isType1Product(productName)) {
      ...
      try {
        ...
        version_extracted_ret = version_extracted_ret.filter(
          (version: any) =>
            !version[0].includes("Release number") &&
            !version[0].includes("Release Number")
        );
        ...
        logger.error(
          "Error extracting versions from JSON when processing Type1 product:",
          { error }
        );
        return version_extracted_ret;
      }
      ...
      try {
        ...
        logger.error(
          "Error extracting versions from JSON when processing Type2 product:",
          { error }
        );
        return version_extracted_ret;
      }
...
async function NotifyNewVersion(newVersion: VersionData, users_array?: any) {
  ...
  const emailBody = EmailBodyCreator(
    "Team",
    `Version Changes Detected: ${newVersion.ProductName.replace(/_/g, " ")}`,
    `Hey Team`,
    `A new version has been detected for ${newVersion.ProductName.replace(
      /_/g,
      " "
    )}`,
    `Version:`,
    ...
    `${
      newVersion.ReleaseDate
        ? newVersion.ReleaseDate.toDateString()
        : "No release date"
    }`
  );
  ...
  try {
    await SendEmail({
      subject: `Version Changes Detected: ${newVersion.ProductName.replace(
        /_/g,
        " "
      )}`,
      content: emailBody,
...
async function createEolVersionToNotify(
  versionInfo: any,
  UsersArray: any,
  daysUntilEOS: number,
  daysUntilExtendedEOS: number,
  eolVersionsToNotify: any
) {
...
async function sendEosEmail(
  users: any,
  frequency: string,
  emailBody: any,
  versionInfo: any,
  IsNotificationTest?: boolean,
  ReturnVal?: { value: boolean }
) {
  ...
  try {
    ...
    if (shouldSendEmail) {
      ...
      for (const email of uniqueEmails) {
        try {
          await transporter.sendMail({
            ...
            to: email as string,
            subject: `End of Support Alert: ${versionInfo.versionData.ProductName.replace(
              /_/g,
              " "
            )} ${versionInfo.versionData.VersionName}`,
            html: createEmailTemplate(
              emailBody,
              versionInfo.versionData.VendorName
            ),
          });
          ...
          logger.info("EOL notification sent:", {
            ...
            nextUpdateTime: nextUpdateTime
              ? new Date(nextUpdateTime).toISOString()
              : "N/A",
      ...
      if (ReturnVal) ReturnVal.value = true;
    } else {
...
async function UpdateLastUpdate(frequency: string) {
  try {
    ...
    const unitOfTime = frequency.includes("_")
      ? frequency.split("_")[0]
      : frequency;
    ...
    if (timeUnitRecord) {
      ...
      logger.warn(
        `Cannot update LastUpdate - TimeUnit not found: ${unitOfTime}`
      );
    }
  } catch (error) {
...
async function SendEmail({
  ...
  if (
    subject.includes("End of Support Alert:") ||
    subject.includes("Critical: End of Support Approaching")
  ) {
    return;
  }
      ...
      for (const mailbox of users_array) {
        ...
        if (shouldSendEmail) {
          ...
          if (affectedCount)
            ...
            logger.error("Error updating last_update in database:", {
              mailbox,
            });
        } else {
          logger.info("Email not sent (last update is not old enough):", {
            mailbox,
          });
        }
      }
